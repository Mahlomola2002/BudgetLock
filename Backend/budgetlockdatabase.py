...
def get_connection(autocommit: bool = True) -> Connection:
    ...
    if r"C:\Users\mahlomola\OneDrive - University of Cape Town\RAG":
...
def insert_task(category_name, amount, emoji):
    try:
        connection = get_connection()

        with connection.cursor() as cursor:
            sql = """
            REPLACE INTO AppData (category_name, amount, emoji)
            VALUES (%s, %s, %s)
            """

            cursor.execute(sql, (category_name, amount, emoji))
...
def display_all():
    try:
        ...
        with connection.cursor() as cursor:
            ...
            print("category_name\tamount\temoji")
            for row in results:
                print(f"{row['category_name']}\t\t{row['amount']}\t{row['emoji']}")
